--[[
	...
	Addon automates the selection of quest and gossip options.
...
function TI_OnEvent(self, event, ...)
	...
	if(TI_status.state and not IsShiftKeyDown()) then 
		...
		if(event == "QUEST_ACCEPT_CONFIRM") then
			TI_debug("Quest Accept Confirm");
			for k,v in pairs(TI_status.options) do
				if(v.type == "escort" and v.state == true) then
					ConfirmAcceptQuest();
		...
		if(event == "LOOT_BIND_CONFIRM") then
			local bopslot = ...
			TI_debug("Loot Bind Confirm");
			for k,v in pairs(TI_status.options) do
				if(v.type == "pickup" and v.state == true) then
					TurnIn:UnregisterEvent("LOOT_BIND_CONFIRM")
					LootSlot(bopslot)
					StaticPopup_Hide("LOOT_BIND");
					ConfirmLootSlot(bopslot)
					TurnIn:RegisterEvent("LOOT_BIND_CONFIRM")
				end	
			end
		end	
		if(event == "CONFIRM_LOOT_ROLL") or (event == "CONFIRM_DISENCHANT_ROLL") then
			local lootid, lootselect = ...
			TI_debug("Confirm Roll");
			for k,v in pairs(TI_status.options) do
				if(v.type == "pickup" and v.state == true) then
					ConfirmLootRoll(lootid, lootselect)
